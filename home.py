...
def app():
    ...
    col3.metric("HCS Goal", "75", "Week 10: -0")

    # Row B
    seattle_weather = pd.read_csv('Orders_Booked.csv')
    stocks = pd.read_csv('Orders_Booked.csv')
    ...
    with c2:
        st.markdown('### Outbound Allocation')
        plost.donut_chart(
        data=stocks,
        theta=donut_theta,
        color='BU',
    ...
    st.line_chart(seattle_weather, x = 'Week', y = 'OLs', height = plot_height)
